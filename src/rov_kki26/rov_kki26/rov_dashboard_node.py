...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Variabel State untuk 2D Trajectory Map
        self.rov_x = 200.0  # Posisi tengah X
        self.rov_y = 150.0  # Posisi tengah Y
        self.rov_v = 0.0    # Kecepatan linear (surge)
        self.map_yaw = 0.0       # Heading map — dead-reckoning dari PWM (gamepad), bukan IMU
        self.rov_yaw_rate = 0.0  # Diisi oleh update_pwm() dari selisih PWM kiri-kanan
        self.path_history = [(self.rov_x, self.rov_y)]
    ...
    def setup_telemetry_row(self):
        ...
        self.lbl_map_canvas = QLabel()
        self.lbl_map_canvas.setFixedSize(400, 300)
        self.lbl_map_canvas.setStyleSheet('background-color: black; border: 1px solid #555;')
        
        # Buat layar gambar / pixmap kosong
        self.canvas_pixmap = QPixmap(400, 300)
        self.canvas_pixmap.fill(Qt.black)
        self.lbl_map_canvas.setPixmap(self.canvas_pixmap)
        
        map_lay.addWidget(self.lbl_map_canvas, alignment=Qt.AlignCenter)
    ...
    def update_3d_render(self):
        ...
        # =========================================================
        # 2. --- UPDATE PENGGAMBARAN TRAJECTORY 2D MAP ---
        # =========================================================
        import math
        from PyQt5.QtGui import QPainter, QPen, QColor, QPolygonF, QBrush, QPixmap
        from PyQt5.QtCore import QPointF, Qt

        # [A] Proteksi Inisialisasi: Mencegah silent-error jika variabel belum ada
        if not hasattr(self, 'rov_x'):
            self.rov_x = 200.0
            self.rov_y = 150.0
            self.rov_v = 0.0
            self.path_history = [(self.rov_x, self.rov_y)]
        if not hasattr(self, 'map_yaw'):
            self.map_yaw = 0.0        # heading map, murni dari dead-reckoning PWM
            self.rov_yaw_rate = 0.0   # diisi oleh update_pwm()

        try:
            # [A2] Integrasikan yaw map dari differential thrust (gamepad),
            #      BUKAN dari IMU — sesuai permintaan: peta full dari gamepad.
            self.map_yaw = (self.map_yaw + self.rov_yaw_rate) % 360.0

            yaw_rad = math.radians(self.map_yaw)
            
            # [B] Kalkulasi pergerakan berdasarkan surge (rov_v) dan yaw
            self.rov_x += self.rov_v * math.sin(yaw_rad)
            self.rov_y -= self.rov_v * math.cos(yaw_rad)
            
            # [C] Batasi batas tembok agar robot tidak bablas hilang dari kanvas (400x300)
            self.rov_x = max(15.0, min(385.0, self.rov_x))
            self.rov_y = max(15.0, min(285.0, self.rov_y))
            
            # [D] Simpan jejak lintasan
            if len(self.path_history) == 0 or (abs(self.rov_x - self.path_history[-1][0]) > 0.5 or abs(self.rov_y - self.path_history[-1][1]) > 0.5):
                self.path_history.append((self.rov_x, self.rov_y))
                if len(self.path_history) > 300: # Batasi memori garis
                    self.path_history.pop(0)

            # [E] BUAT KANVAS BARU: Memaksa UI PyQt untuk refresh setiap frame
            ...
            # Render Garis Lintasan (Warna Hijau)
            ...
            # Render Indikator Robot (Turtle 2D)
            painter.translate(self.rov_x, self.rov_y)
            painter.rotate(self.map_yaw)
            
            # Bentuk menyerupai ROV/Pesawat (Menunjuk lurus ke atas / moncong depan)
            turtle_polygon = QPolygonF([
                QPointF(0, -12),   # Ujung depan
                QPointF(8, 10),    # Sayap Kanan bawah
                QPointF(0, 4),     # Buritan tengah
                QPointF(-8, 10)    # Sayap Kiri bawah
            ])
            
            painter.setBrush(QBrush(QColor(0, 170, 255))) # Warna Biru Muda
            painter.setPen(QPen(QColor(255, 255, 255), 1))
            painter.drawPolygon(turtle_polygon)
            painter.end()
            
            # Terapkan kanvas yang sudah digambar ke Label
            self.lbl_map_canvas.setPixmap(canvas)
            
        except Exception as e:
            # Jika masih ada error, error akan tampil di terminal, bukan membuat layar hitam diam
    ...
    def update_pwm(self, pwm_list):
        ...
        for i, tag in enumerate(tags):
            ...
            # --- KODE YANG HILANG (MENGUPDATE TEKS & WARNA UI) ---
            text = f'{tag}\n{val}'
        ...
        # --- PERHITUNGAN VELOCITY UNTUK MAP ---
        tkiri = pwm_list[1]
        tkanan = pwm_list[4]
        
        # Terapkan deadband agar robot tidak bergerak/drifting saat PWM mendekati 1500
        dorongan_kiri = (tkiri - 1500) if abs(tkiri - 1500) > 20 else 0
        dorongan_kanan = (tkanan - 1500) if abs(tkanan - 1500) > 20 else 0
        
        dorongan_maju = (dorongan_kiri + dorongan_kanan) / 2.0
        
        # Scaling factor dorongan menjadi pixel kecepatan map
        self.rov_v = dorongan_maju * 0.005

        # --- PERHITUNGAN YAW RATE UNTUK MAP (differential thrust) ---
        # Selisih dorongan kanan-kiri menentukan seberapa cepat robot "berbelok"
        # di peta 2D. Ini TERPISAH dari IMU — murni dead-reckoning dari PWM,
        # sesuai kebutuhan: trajectory map full dari gamepad, bukan sensor.
        # Kalau arah putarnya kebalik dari yang dikira, tinggal balik tanda minus ini.
        dorongan_belok = dorongan_kiri - dorongan_kanan
        self.rov_yaw_rate = dorongan_belok * 0.01  # derajat per frame render (33ms)
    ...
    def closeEvent(self, event):
        ...
        for proc_name in ('video_receiver', 'telemetry_receiver', 'qr_scanner', 'mavis_gamepad', 'data_logger'):
            try:
